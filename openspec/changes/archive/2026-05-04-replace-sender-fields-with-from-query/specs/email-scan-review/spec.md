...
The system SHALL expose simple search inputs instead of raw IMAP query syntax. Sender search SHALL use a single generic field, `from_query`, that is mapped directly to IMAP `FROM` search behavior rather than split into separate email-only or display-name-only filters.
